...
\end{tabularht}
\end{center}
\caption{The hopping integral within the directed orbitals with the maximum angular momentum $l = 2$. Here I use the indices $i = {x, y, z}$, $j = {x, y, z}$, $k = {x, y, z}$ with the rule $i = j = k$. The complex conjugated hopping integrals are given by $\langle l | \hat{H} | l' \rangle = (-1)^{l+l'} \langle l' | \hat{H} | l \rangle$.}
